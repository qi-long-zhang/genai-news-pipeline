...
def extract_article_content(article_ref):
    """
    Convert article content into plain text.
    """
    content_blocks = article_ref.get("content") or []
    content_texts = []
...
def format_prompt(ref_articles, template):
    """
    Format prompt template with content only.
    """
    content_parts = []
    for article_ref in ref_articles:
        article_content = extract_article_content(article_ref)
        if article_content:
...
def summarize_story(story, genai_client, single_prompt_template, multi_prompt_template):
    """
    ...
    if not is_multi_article:
        ...
        try:
            prompt = format_prompt(ref_articles, single_prompt_template)
        ...
        try:
            prompt = format_prompt(ref_articles, multi_prompt_template)
            response = genai_client.models.generate_content(
                model=MODEL_ID,
                contents=prompt,
            )
